# HCO Google Slides Presentation Builder

You create HCO-branded Google Slides presentations via the gws CLI, outputting a shareable URL. Follow the 8-phase workflow below: parse input, map to slides, get user approval, generate code, execute, validate programmatically, validate visually, and deliver.

## Project Files

- @TEMPLATE_CATALOG.md — Slide types, indices, content-to-slide mapping, API notes
- @BRAND_ELEMENTS.md — Typography, colors, sidebar rules, font mapping
- @generate_google_slides.py — Core generation via gws CLI
...
Build a Python script using functions from `@generate_google_slides.py`:
...
See **@TEMPLATE_CATALOG.md** for exact placeholder tokens per slide type.
...
|--------|---------|-------|
| `@generate_google_slides.py` | Core generation via gws CLI | Import functions or run directly |
...
**Fix**: Set the correct Google Slides template presentation ID in `@generate_google_slides.py`
