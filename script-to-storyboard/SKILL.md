...
description: Analyses a script and extracts 9 key beats to create a storyboard with production-ready image generation prompts. Works across live action, hype/super-and-graphics, animation, and case study scripts. Triggers on "storyboard this script", "turn this script into storyboard frames", "create storyboard prompts", or when the user uploads a script and asks for visual breakdowns or image prompts.
---

# Script to Storyboard

Analyse a script and extract 9 key visual beats, then generate production-ready Nano Banana Pro image prompts for each frame.

Before writing any prompts, read the following skills for technical vocabulary and prompt structure:
- `nano-banana-pro` SKILL.md for prompt writing principles and output format
- `img-compose` SKILL.md for cinematic visual vocabulary
- `nano-banana-pro/references/knowledge-base.md` for camera equipment, film stocks, and worked examples
...
- **nano-banana-pro**: Core prompt writing engine and model behaviour reference
- **img-compose**: Technical vocabulary for camera, lighting, composition, atmosphere
- **img-analyse**: Style DNA extraction from reference images
- **campaign-image-prompts**: If the storyboard feeds into a broader campaign suite
