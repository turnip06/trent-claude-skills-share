---
name: hco-pptx-deck
description: Creates PowerPoint presentations matching Howatson+Company brand template specifications. Use when building PPTX presentations for Howatson, replicating their design system (VC Nudge Black typography, bold colors, grid layouts), or generating slides with their exact measurements. Covers cover slides, section dividers, content grids, and image layouts.
---

> **When to use:** Use this skill for creating HCO PowerPoint presentations using python-pptx with the template system (exact measurements, grid layouts, automated slide generation). For detailed slide-by-slide PowerPoint design with visual validation, see `create-hco-ppt`. For Google Slides output, see `create-hco-slides`.

# Howatson+Company PowerPoint Template

Create pixel-perfect presentations matching the Howatson+Company design system.

## Quick Start

1. Read `references/template-specs.md` for exact measurements
2. Review `assets/template.pptx` for structure
3. View `assets/screenshots/` for visual reference
4. Install python-pptx: `pip install python-pptx --break-system-packages`
5. Run `python scripts/create_slides.py` to see working examples
...
2. Read `references/template-specs.md` for exact positioning
3. Reference `assets/screenshots/` for visual accuracy
